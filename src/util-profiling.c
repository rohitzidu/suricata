...
static void DumpPrefilterIP(FILE *fp, int ipv, uint64_t total)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        for (int p = 0; p < 256; p++) {
            ...
            fprintf(fp, "%-30s    IPv%d     %3d  %12"PRIu64"     %12"PRIu64"   %12"PRIu64"  %12"PRIu64"  %12s  %-6.2f\n",
                    name, ipv, p, pd->cnt,
...
static void DumpPrefilter(FILE *fp)
{
    ...
    fprintf(fp, "\n%-30s   %-6s   %-5s   %-12s   %-12s   %-12s   %-12s\n",
            "Prefilter", "IP ver", "Proto", "cnt", "min", "max", "avg");
    fprintf(fp, "%-30s   %-6s   %-5s   %-12s   %-12s   %-12s   %-12s\n",
            "--------------------", "------", "-----", "----------", "------------", "------------", "-----------");
